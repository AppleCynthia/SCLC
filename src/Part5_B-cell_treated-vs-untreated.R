...
write.csv(edox, file = file.path( dir_path,  
                                 paste0(use.sub, '.kegg.csv')))

pdf(file = file.path(dir_path,  
                     paste0(use.sub, '.KEGG1.pdf')),20,15)
cnetplot(edox, vertex.label.cex=2,showCategory = 15,circular = TRUE,colorEdge = TRUE)
dev.off()

pdf(file = file.path(dir_path,  
                     paste0(use.sub, '.KEGG1.pdf')),20,15)
barplot(edox,showCategory = 15,font=25)
dev.off()
